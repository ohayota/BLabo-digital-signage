...
// ごみカレンダーに含まれるごみ種別。
// 燃やせるごみ、燃やせないごみ、プラスチック容器包装、カンビンペットボトル。
// Noneはその他として定義。
enum GomiTarget {
  Moyaseru, 
  Moyasenai, 
...
// setup()内で初期化が必要なため、ここではfinalをつけていない（代入できなくなるから）。
color WHITE_COLOR;
...
void setup() {
  ...
  // 初期化用関数initialize()を、画面が描画されているときに並行して実行（非同期処理）。
  thread("initialize");
}
